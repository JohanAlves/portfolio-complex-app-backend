...
exports.apiGetTrending = async function (req, res) {
  try {
    let posts = await Post.getTrending();
    res.json(posts.filter((post, index) => index < 6));
  } catch (e) {
    res.status(500).send("Error");
  }
};

exports.apiGetRecent = async function (req, res) {
  try {
    let posts = await Post.getRecent();
    res.json(posts.filter((post, index) => index < 10));
  } catch (e) {
    res.status(500).send("Error");
  }
};
